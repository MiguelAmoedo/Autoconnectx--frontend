...
const AdicionarPeca = () => {
  ...
  useEffect(() => {
    ...
    const fetchPecaDetails = async () => {
      try {
        ...
        if (response.ok) {
          const { nome, tipoDePeca, marca, modelo, ano, descricao, preco, qtdEstoque, status, partesVeiculo } = data;
          setNome(nome);
  ...
  return (
    <View>
      ...
      <TextInput value={qtdEstoque} onChangeText={setQtdEstoque} />
      ...
      <TouchableOpacity onPress={handleAddPeca}>
